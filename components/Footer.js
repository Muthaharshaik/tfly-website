...
const footerLinks = {
  Services:  ['OSP Survey', 'Field QC', 'Engineering Design', 'Drafting Services', 'OCAL Services', 'Basemap Production'],
  Company:   ['About Us', 'Our Team', 'Why TFly', 'Industries'],
  Resources: ['Get a Quote', 'Contact Us', 'Privacy Policy', 'Terms of Service'],
}
    ...
    <footer style={{
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          {/* Brand — spans full width on tiny */}
          <div style={{ gridColumn: isTiny ? '1 / -1' : 'auto' }}>
            ...
            <div style={{ marginTop: 24, display: 'flex', gap: 10 }}>
              <a href="https://www.linkedin.com/in/tfly-12ab97395/" target="_blank" rel="noopener noreferrer" style={{ padding: '7px 16px', fontSize: 12, color: 'var(--muted)', border: '1px solid var(--border)', borderRadius: 6, textDecoration: 'none', transition: 'all 0.2s' }}
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              ...
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 10 }}>
                {links.map(link => (
                  <li key={link}>
                    <a href="#" style={{ fontSize: 13, color: 'var(--muted)', textDecoration: 'none', transition: 'color 0.2s', fontWeight: 300 }}
        ...
        <div style={{
          ...
          <span style={{ fontSize: 13, color: 'var(--muted-2)' }}>
            © {year} TFly Engineering LLC · tflyind.com · All rights reserved.
          </span>
          <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
            {['Privacy Policy', 'Terms of Service', 'Sitemap'].map(l => (
              <a key={l} href="#" style={{ fontSize: 12, color: 'var(--muted-2)', textDecoration: 'none' }}
                onMouseEnter={e => e.target.style.color = '#fff'}
